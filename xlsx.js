...
if (typeof exports !== 'undefined') {
	if (typeof module !== 'undefined' && module.exports) {
		if(has_buf && typeof jszip === 'undefined') jszip = require('./js'+'zip.js');
...
// returns null on unmerged cell
function getMergeInfo(sheet, col, row) {
	...
	for(var i=0; i < mergedObj.length; i++){
		...
		//console.log("s.c:" + start.c + ", s.r:" + start.r + ", e.c:" + end.c + ", e.r:" + end.r);
		if (start.r <= row && end.r >= row && start.c <= col && end.c >= col){
			//console.log('merged');
...
function getHeaderDepth(sheet, startRow){
	var mergedObj = sheet["!merges"];
	if (!mergedObj){
		return startRow;
	}
...
function getHeaders(sheet, range, headerDepth) {
	// find headers
	var columns = [];
	//console.log("headerDepth: " + headerDepth);
	var temp, previousVal, header;
	for (C = range.s.c; C <= range.e.c; C++) {
		header = "";
		previousVal = "";
		for (R = range.s.r; R <= headerDepth; R++){
			val = sheet[encode_cell({
				c: C,
				r: R
			})];
...
function getColumnsType(sheet, range, headerDepth) {
	var typeOfCols =[];
	...
	for (C = range.s.c; C <= range.e.c; C++) {
		...
		types = [];
		flag = 0;
		for (R = headerDepth + 1; R <= range.e.r; R++){
			val = sheet[encode_cell({
				c: C,
				r: R
			})];
...
function loadTable(sheet, table) {
  ...
  if (mergedCells){
    table.meta.mergedCells = mergedCells;
  } else {
...
function sheet_to_row_object() {
	...
	// fix range
	var sRRange = range.e.r;
	var sCRange = range.e.c;
	var eRRange = range.s.r;
	var eCRange = range.s.c;
	...
	if (tbl.startRow == null ) {
		...
		range.s.r = sRRange;
		tbl.startRow = range.s.r;
	} else {
		range.s.r = tbl.startRow;
	}
	...
	if (tbl.startCol == null) {
		...
		range.s.c = sCRange;
		tbl.startCol = range.s.c;
	} else {
		range.s.c = tbl.startCol;
	}
	...
	if (tbl.endRow == null) {
		for (C = range.s.c; C <= range.e.c; C++){
			for (R = range.e.r; R >= range.s.r; --R) {
				val = sheet[encode_cell({
					c: C,
					r: R
				})];
				//console.log('e r : ' , val);
				if(val !== undefined && R > eRRange){
					//console.log(R);
					eRRange = R;
				}
			}
		...
		range.e.r = eRRange;
		tbl.endRow = range.e.r;
	} else {
		range.e.r = tbl.endRow;
	}
	...
	if (tbl.endCol == null) {
		...
		range.e.c = eCRange;
		tbl.endCol = range.e.c;
	} else {
		range.e.c = tbl.endCol;
	}
	...
	var headerDepth = 0;
	if (tbl.headerDepth == null || tbl.modified.sr == 1) {
		headerDepth = getHeaderDepth(sheet, range.s.r);
	...
	var typeOfCols = [];
	if (tbl.headerTypes == null || tbl.modified.sr == 1 || tbl.modified.sc == 1 || tbl.modified.er == 1 || tbl.modified.ec == 1 || tbl.modified.hd == 1) {
		typeOfCols = getColumnsType(sheet, range, headerDepth);
	...
	var header = [];
	if (tbl.header == null || tbl.modified.sr == 1 || tbl.modified.sc == 1 || tbl.modified.er == 1 || tbl.modified.ec == 1 || tbl.modified.hd == 1) {
		headers = getHeaders(sheet, range, headerDepth);
		tbl.header = headers;
	} else {
		header = tbl.header;
	}
	...
	var rows = [];
	if (tbl.rows == null || tbl.modified.sr == 1 || tbl.modified.sc == 1 || tbl.modified.er == 1 || tbl.modified.ec == 1 || tbl.modified.hd == 1 || tbl.modified.ht == 1) {
		for (var R = headerDepth + 1; R <= range.e.r; R++) {
			var row = [];
			for (C = range.s.c; C <= range.e.c; ++C) {
				var cell = {};
	...
	tbl.modified = {sr: 0, sc: 0, er: 0, ec: 0, h: 0, hd: 0, ht: 0, r: 0};
	obj = {};
	obj.headers = headers;
...
function setStartRow(startRow) {
	...
	this.modified.sr = 1;
	return this.sheet_to_row_object();
	// this.header = null;
...
function makeTbl() {
	var tbl = {
		...
		headerTypes: null,
		rows: null,
		modified: {sr: 0, sc: 0, er: 0, ec: 0, h: 0, hd: 0, ht: 0, r: 0}
	};
	return tbl;
}
